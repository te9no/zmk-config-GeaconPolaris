...
Polaris uses the external
[`zmk-feature-blueboot`](https://github.com/te9no/zmk-feature-blueboot) module
to reboot either XIAO BLE half into the Adafruit nRF52 bootloader's Bluetooth
DFU mode. The behavior is intentionally placed on the `BT` layer:
...
python modules/zmk-feature-blueboot/scripts/blueboot.py path/to/Polaris.uf2
```

This produces `Polaris-blueboot.zip`, which can be transferred with a
...
Polarisは外部モジュール `zmk-feature-blueboot` を利用します。最初の1回は、
このbehaviorを含むUF2をUSBで左右それぞれに書き込みます。
